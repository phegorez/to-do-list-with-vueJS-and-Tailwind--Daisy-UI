        ...
        <div class="cursor-pointer card w-96 bg-neutral text-neutral-content hover:shadow-input" :class="task.status ? 'hover:shadow-success' : 'hover:shadow-error'" v-for="(task, id) in responTask" :key="task.id">
            <div class="card-body items-start text-start relative">
                ...
                <div class="card-actions flex items-center gap-2">
                    <button class="btn btn-primary btn-sm" @click="openEditModal(task.id)" :class="task.status ? 'btn-disabled' : ''">Edit</button>
                    ...
                    <button class="btn btn-error btn-outline btn-sm" @click="deleteTask(task.id)" :class="task.status ? 'btn-disabled' : ''">Delete</button>
                </div>
            </div>
        </div>
    </section>
